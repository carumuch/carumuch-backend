...
- 명시적 요구가 없는 일반 기능 개발에서는 `src/main/java/com/carumuch/capstone/common` 하위 코드를 수정하거나 추가하지 않습니다.
- 공통 설정, 예외 처리, 로깅, 응답 규약처럼 여러 컨텍스트에 실제로 공통인 변경이 필요할 때만 `common`을 수정합니다.
- 도메인 요구사항 해결을 위해 임시로 `common`에 유틸이나 비즈니스 코드를 추가하지 않고, 해당 컨텍스트 내부에서 해결합니다.
...
또한 명시적 공통화 요구가 없는 한 `common` 디렉터리까지 수정 범위를 넓히지 않는 것을 기본 원칙으로 봅니다.
