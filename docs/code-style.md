...
이 문서는 `carumuch-backend`에서 새로 작성하거나 수정하는 코드의 기준을 정의합니다.
...
이 문서는 기본적으로 `src/main/java/com/carumuch/capstone` 이하의 모든 신규 코드에 적용합니다.

레거시 영역은 즉시 전면 수정 대상이 아닙니다.

- `community`
- `bidding`
- `common.legacy`

레거시 영역을 수정할 때는 변경 범위 안에서만 점진적으로 이 문서의 기준에 맞춥니다.
...
최상위 패키지는 `com.carumuch.capstone.<context>` 형태를 사용합니다.
...
4. `infrastructure`는 DB, Redis, JWT, MQ, Mail 같은 외부 기술 구현을 담당합니다
...
- `update`, `changeStatus`, `withdraw`처럼 의도가 드러나는 메서드명을 사용합니다
- `RepairRegion`, `LicenseNumber`, `PhoneNumber`처럼 의미가 있는 값은 VO로 유지합니다
...
- `DamageReportService`, `UserRegisteredEventHandler`, `EstimateResultListener`
- `register`, `update`, `findRecentReports`, `changeStatus`
...
후처리는 핵심 트랜잭션과 분리해야 응답 시간과 장애 범위를 통제할 수 있습니다.

You MUST:

- 트랜잭션 이후 수행할 작업은 이벤트로 분리한다
- 후처리 이벤트는 `@TransactionalEventListener(phase = AFTER_COMMIT)`를 우선 고려한다
- MQ, Mail, 외부 시스템 연동은 핵심 쓰기 트랜잭션 밖에서 처리한다

You SHOULD:

- 이벤트 payload에는 후속 처리에 필요한 값만 담는다
- 재시도, DLQ, 실패 복구 책임은 infrastructure에 둔다

You MUST NOT:

- 저장 트랜잭션 안에서 불필요한 외부 호출을 직접 수행한다
- domain event에 외부 기술 세부사항을 과도하게 노출한다
...
- 검증은 `docs/testing-guide.md`를 따른다
...
이 저장소는 리팩터링된 구조와 레거시 구조가 공존합니다.

You MUST:

- 신규 코드는 리팩터링된 계층 구조를 따른다
- 레거시 코드를 수정할 때는 변경 범위 안에서만 표준으로 수렴시킨다
- 레거시 패턴을 다른 컨텍스트로 복제하지 않는다

You MUST NOT:

- 한 작업에서 관련 없는 전면 리팩터링을 수행한다
- 기존 레거시 구조를 이유로 새 코드까지 같은 방식으로 작성한다
